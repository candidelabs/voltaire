...
async def test_somnia_one_probe_estimation_returns_smallest_success(caplog):
    ...
    with patch(
        ...
        # 1 full-gas probe + 4 grid probes, all against "latest"
...
@pytest.mark.asyncio
async def test_somnia_retry_round_covers_deep_stack_headroom():
    """
...
@pytest.mark.asyncio
async def test_somnia_falls_back_to_legacy_search_when_all_probes_fail():
    """
    If every grid probe of both rounds fails (state drift between probes
    — the retry round's max_call_data_gas candidate cannot fail under
    stable state, since the full-gas probe proved it succeeds), estimation
    falls back to the in-contract binary search.
    """
    gas_manager = _make_somnia_gas_manager()
    user_op = _make_user_operation(_make_execute_user_op_calldata())
