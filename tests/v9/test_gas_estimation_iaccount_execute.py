...
# code path — this covers it against the v9 EntryPoint/bytecode override)
# --------------------------------------------------------------------------
...
async def test_somnia_one_probe_estimation_returns_smallest_success():
    ...
    with patch(
        ...
        # the v9 entrypoint keeps selecting the v9 bytecode override
        state_overrides = mock_rpc.call_args[0][2][2]
        assert state_overrides[ENTRYPOINT_V9]["code"] == \
            gas_manager.entrypoint_code_override_v9
