...
contract EntryPointMod is IEntryPoint, StakeManager, NonceManager, ReentrancyGuard {
    ...
    error SimulationResult(
        uint256 verificationGasLimit, uint48 validAfter, uint48 validUntil, uint256 callGasLimitMax, uint256 numRounds);

    function simulateHandleOpMod(
        UserOperation calldata op,
        EstimateCallGasArgs calldata args) external {
        ...
        revert SimulationResult(opInfo.preOpGas - op.preVerificationGas, data.validAfter, data.validUntil, callGasLimitMax, numRounds);
    }
