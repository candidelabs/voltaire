...
pub const DUPLICATE_CACHE_TIME: Duration = Duration::from_secs(60);
...
#[serde(default)]
/// Network configuration for voltaire.
pub struct Config {
    ...
    /// A setting specifying a range of values that tune the network parameters of voltaire. The
...
impl Default for Config {
    ...
    fn default() -> Self {
        ...
        Config {
            ...
            client_version: voltaire_version::version_with_platform(),
            disable_discovery: false,
            disable_quic_support: true,
            upnp_enabled: true,
            network_load: 3,
            private: false,
...
/// Controls sizes of gossipsub meshes to tune a Voltaire node's bandwidth/performance.
...
/// Return a Voltaire specific `GossipsubConfig` where the `message_id_fn` depends on the current fork.
